    ...
    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <form method="get" class="flex w-full max-w-md items-end gap-2">
            <div class="flex-1">
                ...
                <input type="text" name="q" value="<?= esc($_GET['q'] ?? '') ?>" placeholder="Cari nama partner..." class="admin-input">
            </div>
            <button type="submit" class="admin-btn-primary">Filter</button>
        </form>

        <a href="<?= site_url('admin/partners/create') ?>" class="admin-btn-primary whitespace-nowrap">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50/70">
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-3">
                                    <div class="h-10 w-16 shrink-0 overflow-hidden rounded-lg bg-slate-100">
                                        <img src="<?= base_url($item['logo']) ?>" class="h-full w-full object-contain">
                                    </div>
                                    <span class="font-medium text-slate-700"><?= esc($item['name']) ?></span>
                                </div>
                            </td>
                            <td class="px-4 py-3">
                                <form action="<?= site_url('admin/partners/' . $item['id'] . '/toggle-status') ?>" method="post">
                                    <?= csrf_field() ?>
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="inline-flex items-center justify-end gap-2">
                                    <a href="<?= site_url('admin/partners/' . $item['id'] . '/edit') ?>" title="Edit" class="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-red-100 text-red-700 transition hover:bg-red-200">
                                    ...
                                    <form action="<?= site_url('admin/partners/' . $item['id'] . '/delete') ?>" method="post" data-confirm="Hapus partner ini?">
                                        <?= csrf_field() ?>
                    ...
                    <?php if (empty($items)): ?>
                        <tr><td colspan="3" class="px-4 py-10 text-center text-slate-400">Belum ada data partner.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>

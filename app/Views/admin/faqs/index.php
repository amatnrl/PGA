    ...
    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <form method="get" class="flex w-full max-w-md items-end gap-2">
            <div class="flex-1">
                ...
                <input type="text" name="q" value="<?= esc($_GET['q'] ?? '') ?>" placeholder="Cari pertanyaan..." class="admin-input">
            </div>
            <button type="submit" class="admin-btn-primary">Filter</button>
        </form>

        <a href="<?= site_url('admin/faqs/create') ?>" class="admin-btn-primary whitespace-nowrap">
                ...
                <tbody class="divide-y divide-slate-100">
                    <?php foreach ($items as $item): ?>
                        <tr class="hover:bg-slate-50/70">
                            <td class="px-4 py-3 font-medium text-slate-700"><?= esc($item['question']) ?></td>
                            <td class="px-4 py-3 text-slate-500"><?= esc($item['category'] ?? '-') ?></td>
                            <td class="px-4 py-3">
                                <form action="<?= site_url('admin/faqs/' . $item['id'] . '/toggle-status') ?>" method="post">
                                    <?= csrf_field() ?>
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="inline-flex items-center justify-end gap-2">
                                    <a href="<?= site_url('admin/faqs/' . $item['id'] . '/edit') ?>" title="Edit" class="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-red-100 text-red-700 transition hover:bg-red-200">
                                    ...
                                    <form action="<?= site_url('admin/faqs/' . $item['id'] . '/delete') ?>" method="post" data-confirm="Hapus FAQ ini?">
                                        <?= csrf_field() ?>
                    ...
                    <?php if (empty($items)): ?>
                        <tr><td colspan="4" class="px-4 py-10 text-center text-slate-400">Belum ada data FAQ.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
